...
@page :first { size: A5 landscape; margin: 5mm; }
...
12. **實機印表機列印**(挑一台辦公室常用機):A5 紙列印,表格邊框、圓角、visitSerial 都印得出來;若 B&W 老印表機 #AAAAAA halftone 太糊,記錄下來(後續微調)
...
2. **`buildA5Page` 沒走 4-column 路徑,所以 `buildColumn`/`buildSectionBox` 都不用** — 但 `buildResultMap` / `genderFilteredTests` / `valueStyle` 等 helper 仍要重用,別重複寫
3. **A5 紙印表機支援**:確認辦公室印表機 A5 紙匣 / 手動進紙正常,否則做完使用者印不出來
