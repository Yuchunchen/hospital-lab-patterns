...
| Trigger | 對應 SOP | 何時用 |
|---|---|---|
| 「告一段落」/「階段完成」/「先停一下」/「休息一下」 | SOP G — Wrap | 工作斷點,可能同對話繼續或之後接續;不必開新對話 |
| 「離開 vhyl」/「離開 vhtt」/「換到 vhtt」/「換到 vhyl」/「換機器」 | SOP H — Leave machine | 要離開本台機器(關機 / 出門 / 換機器),可能跨機接續 |
| 「接續 vhtt」/「接續 vhyl」/「接續上次」/「繼續上次」 | SOP I — Resume | 開新 session 要接續上一段(同機或跨機)|
| 「準備開新 session」/「等等要重開對話」 | SOP J — End session | 明確要結束本 Cowork 對話,要新對話的開場句 |
...
> 你的意思是哪一種?
> (a) 工作告一段落想休息,本對話可能還會繼續 → 跑 SOP G(寫 snapshot + 給 git 指令)
> (b) 要結束本 Cowork 對話、之後開新對話接續 → 跑 SOP G + SOP J(加新 session 開場句)
> (c) 要離開本台機器 → 跑 SOP H(G + 檢查 handoff)

理由:這類口語含糊度高、猜錯成本大(誤跑 J 會浪費你開新對話的力氣;誤跑 G 會漏 handoff 寫作)。寧可多問一次,不要 silent drift。
